 ...
 * - run getDriveIdObj function to get drive ID of every year and every country (only 2023+) for drive J and drive Lock
...
function lockFolderOnTrigger() {
  try {
    let j = getDriveIdObj("J Folder");
    let lock = getDriveIdObj("Lock Folder");
    ...
    projectArr.forEach((project) => {
      if (parseInt(project.year) > 2022) {
        ...
        for (let i = 0; i < jIdKeys.length; i++) {
          ...
          foundProject = loopProjectFolders(
            j[year][countryKey],
            lock[year][countryKey],
            projectno
          );
        ...
        console.log("<=22");
        let foundProject = loopProjectFolders(
          j[project.year],
          lock["Lock 2"],
          project.projectno
        );
    ...
    console.log("moveFolder error : " + e.message);
  }
}
 ...
 */
function getDriveIdObj(sheetname) {
  let jFolderIdData = SpreadsheetApp.openById(ruleSheetId)
...
function getProjectArr() {
  try {
    ...
    var threads = GmailApp.search(
      'from:dwpservice@dwp.com AND  {subject:"AR Notification (45 Days Overdue)" subject:"AR Notification (60 Days Overdue)"} AND is:unread'
    )[0];
    ...
    Logger.log("getProjectArr error : " + e.message);
  }
}
...
function loopProjectFolders(jFolderId, lockFolderId, projectno) {
  ...
  while (folders_j.hasNext()) {
    ...
    if (folderName.includes(projectno)) {
      ...
      console.log(
        `folder ${folderName} - J drive = ${jFolderId} moveTo folders_Lock = ${lockFolderId}`
      );
